...
(function($) {
    ...
    function Plugin($container, options) {
        ...
        function _dashedCircle(ctx, x, y, radius, offset, segments, size) {
            var pi2 = 2 * Math.PI,
                corcleTop = (Math.PI*1.5),
                segs = pi2 / segments,
                len = segs * size,
                i = corcleTop,
                ax, ay;
        ...
        function _initCanvas() {
          ...
          $overlay.prepend(newCanvas);

          var canvas = $overlay.find('canvas').get(0);
          var ctx = canvas.getContext('2d');

          ctx.canvas.width  = containerSize.width
          ctx.canvas.height = containerSize.height
          ...
          if (canvas.getContext) {
            ...
            _dashedCircle(ctx, radius, radius, radius -10, _toRadians(dotSize.width/(self.slidesTotal/3)), self.slidesTotal, defaults.gapBetwean);
       ...
       /**
        ...
        function _setCSS(angle, fireCallback) {
            if (self.slideLast !== self.slideCurrent ) {
              ...
              $slides.filter(':visible').fadeOut(1000).css({zIndex: 0});

              $slides.eq(self.slideCurrent).css('zIndex', 50 + 1).fadeIn(1000, function() {
                $(this).css('zIndex', 50);
              });

              self.slideLast = self.slideCurrent;
            }
